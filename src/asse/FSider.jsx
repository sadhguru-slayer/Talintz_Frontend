...
const FSider = ({ 
  ...
  return (
    ...
    >
      ...
      <div className="w-full px-4 py-4 border-t border-violet-100">
        <motion.button
          whileHover={{ scale: 1.02 }}
          whileTap={{ scale: 0.98 }}
          onClick={() => setIsModalOpen(true)}
          className={`
            w-full flex items-center gap-3.5 p-2.5 rounded-lg 
      ...
      <Modal
        title={logoutStep === 'initial' ? "Confirm Logout" : "Logging Out"}
        open={isModalOpen}
        onOk={handleLogout}
      ...
      >
        {logoutStep === 'processing' ? (
          <div className="flex flex-col items-center justify-center py-4">
            <div className="w-8 h-8 border-4 border-freelancer-primary border-t-transparent rounded-full animate-spin mb-4" />
            <p className="text-gray-600 text-center">Logging you out securely...</p>
          </div>
        ) : logoutStep === 'done' ? (
          <div className="flex flex-col items-center justify-center py-4">
            <div className="text-green-500 text-2xl mb-4">✓</div>
            <p className="text-gray-600 text-center">Logout successful!</p>
          </div>
        ) : (
