...
import { Card, Button, Tag, Avatar, Rate, Steps, message, Modal, Input } from 'antd';
...
const StandardAssignment = ({ 
  ...
  const getSteps = () => {
    const baseSteps = [
      ...
      { 
        title: 'Shortlist', 
        description: 'Select top candidates', 
        icon: <StarOutlined />,
        key: 'shortlist'
      }
    ];
    ...
    if (includeInterview) {
      baseSteps.push(
        { 
          title: 'Interview', 
          description: 'Optional discussions', 
    ...
    try {
      ...
      if (isCurrentlyShortlisted) {
        ...
        if (success.data.status === 'success') {
          message.success('Bid removed from shortlist');
          onRefreshBids();
        }
      } else {
        ...
        if (success.data.status === 'success') {
          message.success('Bid shortlisted');
          onRefreshBids();
        }
      }
    } catch (error) {
  ...
  const handleNextStep = () => {
    const maxStep = includeInterview ? 3 : 2;
  ...
  return (
    ...
    >
      ...
      <div className="bg-white/10 backdrop-blur-xl rounded-xl p-6 border border-white/10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-semibold text-white">Standard Assignment</h2>
            </div>
        ...
        <div className="mt-4 p-4 bg-white/5 rounded-lg border border-white/10">
          <div className="text-sm text-white/80">
            ⏱️ 3-5 days • Shortlist & review • Balanced approach
          </div>
        </div>
      </div>
      ...
      <div className="bg-white/10 backdrop-blur-xl rounded-xl p-6 border border-white/10">
        ...
        <div className="flex justify-between items-center">
          <div className="text-white/60">
            ...
            {getCurrentStepKey() === 'shortlist' && `${bids.filter(bid => bid.state === 'under_review').length} candidates shortlisted`}
            {getCurrentStepKey() === 'interview' && 'Interview phase in progress'}
            {getCurrentStepKey() === 'final' && (assignedBid ? 'Project assigned' : 'Waiting for assignment')}
          </div>
          ...
          <div className="flex gap-3">
            ...
            {currentStep < steps.length - 1 && (
              <Button 
                ...
                disabled={
                  (getCurrentStepKey() === 'review' && bids.length === 0) ||
                  (getCurrentStepKey() === 'shortlist' && bids.filter(bid => bid.state === 'under_review').length === 0)
                }
              >
                Next Step
              </Button>
      ...
      <AnimatePresence mode="wait">
                          ...
                          <div className="text-right">
                            ...
                            <Button
                              ...
                              className={bid.state === 'under_review' 
                                ? "bg-client-accent hover:bg-client-accent/90 shadow-none border-0" 
        ...
        {getCurrentStepKey() === 'interview' && includeInterview && (
          <motion.div
            key="step-interview"
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
          ...
          >
            <div className="bg-white/10 backdrop-blur-xl rounded-xl p-6 border border-white/10">
              ...
              <p className="text-white/60 mb-4">
                ...
                Review their responses and make your final selection.
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Optionally show declined interviews (for transparency) */}
                {bids.filter(bid => bid.state === 'interview_declined').length > 0 && (
                  <div className="col-span-full">
                    <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                      <h4 className="text-white/60 text-sm mb-3">Declined Interviews</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {bids
                          .filter(bid => bid.state === 'interview_declined')
                          .map((bid) => (
                            <div 
                              key={bid.id}
                            ...
                            >
                              <div className="flex items-center gap-3">
                                <Avatar 
                                  size={40} 
                                  src={bid.freelancer.avatar}
                                  className="ring-1 ring-red-500/30"
                                />
                                ...
                                <Tag className="bg-red-500/20 text-red-400 border-red-500/30 text-xs">
                                  Declined
                                </Tag>
                              </div>
                            </div>
