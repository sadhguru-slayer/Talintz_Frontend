...
const FreelancerProjectWorkSpace = ({ userId, role,isAuthenticated, isEditable }) => {
  ...
  useEffect(() => {
    const fetchProjectData = async () => {
      ...
      try {
        ...
        const projectData = {
          id: projectId, // Use the provided project ID
          title: `Project ${projectId} - E-commerce Website Development`,
  ...
  return (
      ...
      <div className={`${isMobile ? 'ml-0' : 'ml-14'}  flex-1 flex flex-col overflow-hidden `}>
        ...
        <div className={` flex-1 overflow-auto p-4 md:p-6`}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative overflow-hidden rounded-2xl mb-6 shadow-xl">
            <div className="absolute inset-0 bg-gradient-to-br from-freelancer-primary via-freelancer-primary/80 to-freelancer-bg-dark"></div>
            <div className="absolute inset-0 bg-freelancer-gradient-soft opacity-30"></div>
            <div className="absolute top-0 right-0 w-48 h-48 bg-freelancer-accent/10 rounded-full blur-3xl -mr-24 -mt-24 animate-float"></div>
            ...
            <div className="relative z-10 p-6 sm:p-8">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span className="text-white/90 text-lg">
                      <UserOutlined className="mr-2" /> You
                    </span>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 pt-6 border-t border-white/10">
                {[
                  { icon: <ProjectOutlined />, label: "Milestones", value: `${milestones.filter(m => m.completed).length}/${milestones.length}`, color: 'var(--freelancer-accent)' },
                  { icon: <DollarOutlined />, label: "Budget", value: `$${project?.amountPaid?.toLocaleString()}`, color: 'var(--status-success)' },
                  { icon: <CalendarOutlined />, label: "Timeline", value: `${Math.ceil((new Date(project?.deadline) - new Date()) / (1000 * 60 * 60 * 24))} days`, color: 'var(--freelancer-primary)' },
                  { icon: <TeamOutlined />, label: "Team", value: "4 Members", color: 'var(--freelancer-secondary)' }
                ].map((stat, i) => (
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}
            className="rounded-2xl border border-white/10 bg-gradient-to-br from-freelancer-secondary/40 to-freelancer-bg-dark/30 backdrop-blur-xl shadow-xl"
          >
            <Tabs
              activeKey={activeTab}
            ...
            >
              ...
              >
                    ...
                    <div className="lg:col-span-2">
                      <div className="bg-gradient-to-br from-freelancer-secondary/40 to-freelancer-bg-dark/30 backdrop-blur-xl rounded-xl border border-white/10 p-4 shadow-md">
                        ...
                        <h3 className="text-xl font-bold text-white mb-4 cred-font">Upcoming Milestones</h3>
                            ...
                            <motion.div key={milestone.id} whileHover={{ x: 4 }} className="p-4 rounded-xl border border-white/10 hover:bg-white/10 transition-all duration-300">
                              ...
                              <div className="flex justify-between items-center mt-2 text-sm">
                                ...
                                <span className="font-semibold text-freelancer-accent">${milestone.amount}</span>
                              </div>
                            </motion.div>
                          ))}
                                  </div>
                  ...
                  <div className="space-y-4">
                    {milestones.map(milestone => (
                      <motion.div key={milestone.id} whileHover={{ x: 4 }} 
                        className="rounded-xl bg-gradient-to-br from-freelancer-secondary/40 to-freelancer-bg-dark/30 backdrop-blur-xl border border-white/10 p-4 shadow-md overflow-hidden"
                      >
    ...
    <div>
                          ...
                          <div className="flex items-center gap-4">
                            <span className="font-semibold text-freelancer-accent text-lg">${milestone.amount}</span>
                                        {!milestone.completed && (
                                            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                                              size="small"
                ...
                <div className="p-6">
                  <div className="border-2 border-dashed border-white/30 rounded-xl p-8 text-center bg-white/5 backdrop-blur-sm shadow-md">
                    ...
                    <p className="text-white font-medium mb-2">
                      Drag files here or <span className="text-freelancer-accent cursor-pointer">browse</span>
                                </p>
...
export default FreelancerProjectWorkSpace;